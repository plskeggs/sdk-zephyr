		      ...
		      int line, const char *str)
{
	...
	char *log = log_strdup(str);

	/* Remove extra EOL to improve log readability. */
	log[strlen(str) - 1] = '\0';
	NET_DBG("%s:%04d: |%d| %s", basename, line, level, log);
}
...
static int tls_opt_dtls_connection_id_get(struct tls_context *context,
					  void *optval, socklen_t *optlen)
{
#if defined(CONFIG_MBEDTLS_SSL_DTLS_CONNECTION_ID)
	struct tls_dtls_peer_cid *val = (struct tls_dtls_peer_cid *)optval;

	__ASSERT(sizeof(val->peer_cid) == MBEDTLS_SSL_CID_OUT_LEN_MAX, "peer cid size mismatch");
			...
			void *optval, socklen_t *optlen)
{
	...
	case TLS_DTLS_CONNECTION_ID:
		err = tls_opt_dtls_connection_id_get(ctx, optval, optlen);
		break;
#endif /* CONFIG_NET_SOCKETS_ENABLE_DTLS */
